...
const configFileNames = ["openai-compact.json", "openai-compact.jsonc"];
...
export function getGlobalConfigSources() {
...
function findNearestOpencodeDir(start) {
...
export function getConfigSources(context) {
    const sources = [...getGlobalConfigSources()];
    if (process.env.OPENCODE_CONFIG_DIR)
        sources.push(...configFilesIn(process.env.OPENCODE_CONFIG_DIR));
    const projectConfigDir = findNearestOpencodeDir(context.directory);
    if (projectConfigDir)
        sources.push(...configFilesIn(projectConfigDir));
    return sources;
}
